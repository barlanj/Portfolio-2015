 ...
 * Author: barlanj
 * Description:
 *      This is a server program for encrypting a plain text.
 *
 * Sources Used:
...
char * encrypt_msg(char message[], char key[]);
...
void doprocessing( int sock )
{
    ...
    cipher_msg = encrypt_msg(plain, key);
...
char * encrypt_msg(char message[], char key[])
{
    ...
    for(i=0; i < msg_len; i++) {
        ...
        if(isalpha(message[i]) && isupper(message[i]) && isalpha(key[i])) {
            k = key[i] - 'A';
            cipherText[i] = ((message[i] - 'A' + k) % 26) + 'A';
        }
        else
        {
